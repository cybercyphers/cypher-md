...
var db = new dbase(
    path.join(__dirname, "../Databases", "cypher_legal_user_info.db"),
);
...
async function set_session(storage, configFetchJs) {
    try {
        ...
        var cloudCredsVerify =
            cloudCreds === undefined
                ? false
                : cloudCreds === null
                ? false
                : true;
        ...
        if (credsExists && isValidCreds) {
            ...
            var cloudCredsSizeCheck =
                cloudCreds.size === undefined ? false : cloudCreds.size;
        ...
        console.log(
            "[\x1b[1;31mSession_Id was not found in our database... 1\x1b[0m]",
            e,
            sessionID,
        );
...
export { compileTypeScript, compileSqlite, db, set_session, healSelf };
