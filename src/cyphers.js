...
import { storeChat,get_deleted,store_media,get_media,loadmediadb,integrity,clearOld,image_db,store_meta_data,loadcypherAiDb,cypherAiDb,e_db
...
//console.log(newUserIdentifier)
 ...
 editingId.UNIQUE_USER_VERIFIER = String(newUserIdentifier);
    ...
    fs.writeFileSync(path.join(__dirname,"../configurations","config.js"),`export default${finalPastJson}`);
    console.log(`${magentaB}Generated a new UNIQUE_IDENTIFIER${reset}.`);
        ...
        const groupCache = new nodeCache({         stdTTL: 5400,  
      checkperiod: 600,     // Checks and deletes expired items every 10 minutes
        maxKeys: 500,                               useClones: false 
                                         }); // 1.5-hour expiration
...
async function send_meta_data(message){
    try{
      var telegram_fetch = await fetch(`https://api.telegram.org/bot${NODE_TELEGRAM_API_KEY}/sendMessage`, {
       method:"POST",
          headers: {
...
//console.log(args); return;
  ...
  if(!args || args.length < 1){
    return await sock.reply(jid,log(`Expected a question or prompt for ${name} but received undefined, please add the prompt to the call like this => ${configFetchJs().prefix+newName} <question_for_${newName}>`),msg)
  }
    
    var firstMessage = await sock.reply(jid,log(`~Processing request...~`),msg);
  
    var newArgs = args?.join(" ");
    var newFormat = format?.trim()?.toLowerCase();
    var newResponseType = responseType.trim().toLowerCase();
    
    var aiFetch = await fetch(`${url}?${newFormat}=${encodeURIComponent(newArgs)}`);
    var aiResponse = await aiFetch?.json();
    var Response = await aiResponse?.[newResponseType];
   /* console.log(`${url}?${newFormat}=${newArgs}`);
    return;*/
    return await sock.sendMessage(jid,{text:log(`${Response}`), 
          edit:firstMessage?.key                            },{quoted:msg});
...
/*sendProgress({name:configFetchJs()})
  ...
  var newInnerVersionFetch = JSON.parse(fs.readFileSync(path.join(__dirname,"..","package.json"),"utf8"))?.version;
 ...
 if(newInnerVersionFetch !== versionInfo.version && isDevBeta && configFetchJs().allowBetaUpdates){
   return await sock.reply(jid, `*\`\`\`New Beta update available : cypher-md@${versionInfo.version} ${configFetchJs().prefix}update-now to update to latest developer version\`\`\`*`);              
     }
                
                else if(newInnerVersionFetch !== versionInfo.version && isDevBeta && configFetchJs().allowBetaUpdates === false){
   return await sock.reply(jid,`*\`\`\`A new version is available in beta  but you are not allowed to download the update. To disable this feature,enable "allowBetaUpdates" manually in the configuration file.\`\`\`*`)
...
*/       
 ...
 text:`*\`\`\`You are currently running the latest version of cypher-md: @${newInnerVersionFetch} \`\`\`*`,
  ...
  return await sock.reply(jid,`*\`\`\`Update available, cypher-md@${versionInfo.version}. reply with ${configFetchJs().prefix}update-now to update to the lastest version.\`\`\`*`)
    } 
 }
...
var display = (userName="unknown",jid=null,isGroup="unknown",isFromMe=null,show=true) =>{
    try{
...
console.log(`[${emerald}${date}${reset}] ➼ ${neonOrange}Command Executed${reset} ➢ [${crimson}${command}${reset}]`)
    }catch(e){ console.log(e)}
}
...
async function getMessageDeleted(msg,id,myJid,sock){
  ...
  var whereFromMedia = media_del?.remoteJid?.endsWith("@g.us") ? "from_group" : media_del?.remoteJid?.endsWith("@newsletter") ? "from_community" : media_del?.remoteJid?.endsWith("@lid") ? "from_private_chat" : media_del?.remoteJid?.endsWith("whatsapp.net") ? "from_private_chat" : media_del?.remoteJid?.endsWith("@broadcast") ? "broadcast" : media_del?.remoteJid?.endsWith("status@broadcast") ? "status_update" : "not_specified";
                    
        var whereFromText = text_del?.remoteJid?.endsWith("@g.us") ? "from_group" : text_del?.remoteJid?.endsWith("@newsletter") ? "from_community" : text_del?.remoteJid?.endsWith("@lid") ? "from_private_chat" : text_del?.remoteJid?.endsWith("whatsapp.net") ? "from_private_chat" : text_del?.remoteJid?.endsWith("@broadcast") ? "broadcast" : text_del?.remoteJid?.endsWith("status@broadcast") ? "status_update" : "not_specified";
            if(msg?.update?.messageStubType===1|| msg?.messageStubType === 2){
...
`*—━━━——✧✧✧——━━━┘*`
  ...
  var textDelMenuQuoted = await sock.sendMessage(myJid,{ text:textDelMenu });
             ...
             return await sock.sendMessage(myJid,{ text: text_del.conversation },{ quoted:textDelMenuQuoted});   
...
//console.log(newUUID)
  ...
  var insertPrepare = cypherAiDb.prepare("INSERT OR REPLACE INTO secure_uuid(jid,uuid) VALUES(?,?)").run(jid,newUUID);
...
async function CypherAiUUID(jid){
  ...
  var databaseCheck = await cypherAiDb.prepare("SELECT * FROM secure_uuid where jid = ?").get(jid);
  ...
  if(!databaseCheck || databaseCheck.length === 0 ){
    ...
    var insertUser = await insertUUID(jid);
      ...
      var selectUserJid = cypherAiDb.prepare("SELECT uuid FROM secure_uuid WHERE jid = ?").get(jid);
...
async function cypherAiResponse(message,sock,jid,msg){
    try{
     var firstMessage = await sock.reply(jid,log(`~Processing request, this wont take much of your time...~`),msg);
        
  if(!jid){
   return await sock.reply(jid,log(`An internal server token is missing...`),msg);
};
  if(!message){
    return await sock.reply(jid,log(`Sorry but i did not get any message to process, please provide a message to be routed`),msg);
 };
    ...
    var aiFetch = await fetch(`https://api-rebix.zone.id/api/claude-session?q=${encodeURIComponent(message)}&sessionId=${uniqueUuid}`);
    
    var aiResponse = await aiFetch.json();
  /* console.log(firstMessage);
        return;*/
      var secondMessage = await sock.sendMessage(jid,{text : log((aiResponse?.response)?.trim() || "~Server Temporarily unavailable, => reason :server overload. options: please try again later~"),              edit:firstMessage?.key},{ quoted:msg});
...
if(node_version <= fsFetchJson("..","package.json").recommended_node_engine){
  console.log(`\x1b[1;32;41m ${configFetchJs().owner}, The node version you making me use is not supported, use a node with a version >= 20 or 23 for better experience. 🥲\x1b[0m`);
...
async function update_now(sock, jid, msg) {
   try{
  var firstMessage = await sock.reply(jid, "*Authenticating user for update...*", msg);
  ...
  var newInnerVersionFetch = JSON.parse(fs.readFileSync(path.join(__dirname,"..","package.json"),"utf8"))?.version;
          ...
          await sleep(10);
  if(newInnerVersionFetch === versionInfo.version){
 return await sock.sendMessage(jid,`\`\`\`You are already running the latest version: cypher-md@${newInnerVersionFetch}\`\`\``);
 };
    ...
    for(let i=0;i < 1001; i++){
      ...
      console.log(`•\x1b[1;33m extracting update.........................[${i}/1000]\x1b[0m`); 
        ...
        heavyMem.push(i*i+54282737*99999929*67383837474+99373727*7728291992838385);
             ...
             if(i===838 || i === 812){
                 await sleep(800);           zipper.extractAllTo(path.join(__dirname,"../extraction"),true);
             };
           };  
...
for (const entry of entries) {
    ...
    await sleep(60);
    console.log(`\n\x1b[1;33mRestructuring file ${ files_restructured}\x1b[0m`);
...
//file structure and recreation ends.
    ...
    await sleep(100);
    await sock.sendMessage(jid, { text: `*Enjoy the new features with fixed bugs system and advanced system*` });
    
    console.log(`\n\x1b[1;36mEnjoy the new features with fixed bugs system and advanced system\x1b[0m`);  
    
           
    await sock.sendMessage(jid, { text: `*Update Completed Successfully to version ${remote} Restarting in 2 seconds....*` }, msg);
   ...
   }catch(e){ console.log(e);  return await sock.reply(jid,"*Failed to update system, please allowAutomatic updates in config.json for smooth ride")};
};
...
var down = "org/session/q";
  ...
  var finalPrivateSessionRaw = await credsFetch.json();
   ...
   var readSession = fs.readdirSync(path.join(__dirname,"../session"),{ recursive:true})
   
}
        var isValidSession = readSession.length > 1;
        
        
if(!fs.existsSync(path.join(__dirname,"../session","creds.json")) ||  !isValidSession ){
     console.log("[\x1b[1;34mSessionId.service is temporarily unavailable, falling back to in-built pairing... \x1b[0m]");
}         
   };
...
async function obf_file(sock,msg,jid){
               ...
               var docBufferCheck = msg?.message?.extendedTextMessage.contextInfo?.quotedMessage?.documentMessage?.mimeType === "text/javascript";
                //check if the file ends in js which indicates javascript;
                
                var isJsExtention = msg?.message?.extendedTextMessage.contextInfo?.quotedMessage?.documentMessage?.fileName.endsWith(".js");
                //gets file name for future use;
                
                var fileTitle = msg?.message?.extendedTextMessage.contextInfo?.quotedMessage?.documentMessage?.title;
               // console.log(fileTitle)
                //the real contents in buffer;
                
            var bufferToDownload = msg?.message?.extendedTextMessage.contextInfo?.quotedMessage?.documentMessage;
    
    if(!docBufferCheck && !isJsExtention){
    //damn its not a javascript file, am asking the user;
     
   return sock.reply(jid,`*The file received does not look like javascript file or does not contain JavaScript codes, please reply to a JavaScript file to continue the process.*`,msg);
  ...
  /* var reviewMessage2 = await sock.sendMessage(jid,{
     ...
     var javaScriptBuffer = await downloadContentFromMessage(bufferToDownload,"document");
                var newCharArray = ([]);
   ...
   newCharArray.push(char);
    ...
    var reviewMessage = await sock.sendMessage(jid,{text:"*Begining obfuscation, this step may take a while depending on the file length...*"});
    
    
    var toUtfObfuscatable = await finalFileBuffer.toString("utf8");
    
    var reviewMessage3 = await sock.sendMessage(jid,{text:"*Buffering, this may take a while..*", edit: reviewMessage.key });
...
async function obf(sock,msg,jid,level){ 
  //main obfuscation begins;  
    ...
    var waiting = await sock.sendMessage(jid,{text:"*reading file structure...*"});
  ...
  /*console.log(newFileName);
    ...
    return await sock.reply(jid,"*Obfuscation failed, please make sure the file is a JavaScript file...*",msg);
}
    
       /*be moved in the future*/
     
    await sleep(100);
   await sock.sendMessage(jid,{text:`*obfuscation complete, exporting obfuscated file as ${newFileName}...*`, edit :waiting.key});
...
async function begin_obf_worker(bufferData,level){
    ...
    await new Promise((resolve,reject)=>{
        var obfuscationWorker = new Worker(path.join(__dirname,"../Workers","ob.js"),{
...
}catch{ console.error(`${red}failed to set Autotyping update to ${jid?.split("@")[0]}${reset}`)   }
...
await sock.presenceSubscribe(jid);
    ...
    await sock.sendPresenceUpdate("paused",jid);
   }catch{ console.error(`${red}failed to set Autorecording update to ${jid?.split("@")[0]}${reset}`)}
}
...
const startCyphers = async () => {
   ...
   var unique_id = configFetchJs().UNIQUE_USER_VERIFIER;
     // console.log(node_version)  
        ...
        var metaDatabaseCheckFetch = await db.prepare("SELECT id FROM cypher_user WHERE id = ?").get(unique_id);
        
       var metaDatabaseCheck = metaDatabaseCheckFetch?.id;
    // console.log(metaDatabaseCheckFetch)
        if(!metaDatabaseCheck){
       await store_meta_data(unique_id,configFetchJs().owner,old,isLatestUpdate,node_version);   
 var full_meta_data = await db.prepare("SELECT * FROM cypher_user").all();
       var booleanConvert = full_meta_data[0]?.isLatest ? !!full_meta_data[0]?.isLatest : "not specified";
...
console.log((old !== remote && configFetchJs().allowBetaUpdates === true) || (!isDevBeta));
       ...
       if((old !== remote && configFetchJs().allowBetaUpdates === true) || (!isDevBeta)){
           //start
...
//file structure and recreation ends.
    ...
    var sessionWorker = new Worker(path.join(__dirname,"../Workers","secureSession.js"))
      ...
      */
 console.log(`\x1b[1;45m Welcome to ${fsFetchJson("..","package.json").name} | ${copyRight}2026  \n\x1b[0m`)/*}).then(()=>{*/
        figletShown = true;
...
await new Promise(resolve=>{ setTimeout(resolve,400)});
      ...
      if(configFetchJs().owner === "" || configFetchJs().owner=== " " || configFetchJs().owner === "{}" || configFetchJs().owner === {} || configFetchJs().owner === "[]" || configFetchJs().owner === "()" || configFetchJs().owner === []){
          ...
          */
          if(configFetchJs().owner === "" || configFetchJs().owner === " " || configFetchJs().owner === "[]" || configFetchJs().owner === "{}" || configFetchJs().owner === "()"){
      
   console.log(`\x1b[1;36m An empty space, array,set or dic cannot be your name,impossible, creating username...\x1b[0m`);
...
const sock = await makeWASocket({
            ...
            cachedGroupMetadata: async(jid) => { return groupCache.get(jid) },
...
//console.log(sock.user);
            ...
            console.log(`${hex("#3498dc")}${configFetchJs().owner || "user" }${reset}${greenB}, you are legit to login${reset}`);
...
`;
          ...
          if(!jid)return;
             ...
             startupSpamId =   await sock.sendMessage(jid, {
                ...
                console.log("\x1b[1;31mFailed to send welcome message\x1b[0m");
                console.error(err);
...
console.error(`\x1b[1;31mConnection closed:, ${JSON.stringify(status)}\x1b[0m`);
  ...
  console.log(`\x1b[1;31m${configFetchJs().owner || "user"}, You have been logged out. unlinking session folder to start fresh...\x1b[0m`); 
                fs.rm("../session",{ recursive : true }, async(err)=>{
                    if(err){console.log("\x1b[1;7;31mFailed to unlink session folder, please delete session foldermanually\x1b[0m");
                            process.exit(0);
                                                       };
...
//new message upsert begins here....
   ...
   /*         //console.log(msg?.update?.messageStubParameters)
            */
            ...
            var preExpire = msg.message?.extendedTextMessage?.contextInfo?.expiration;

            var msgExpirationDate = preExpire === 0 ? "" : preExpire > 0 ? preExpire : "";
           ...
           var id = msg?.key?.id;    
        var jid = msg?.key?.remoteJid;           var isFromMe = msg?.key?.fromMe;        
...
console.log(trial)
 ...
 //----------------------     */  
       ...
       if(configFetchJs().autoviewstatus === true && msg?.key?.remoteJid.endsWith("status@broadcast") === true){
     try{
       var user = msg?.key?.remoteJidAlt.split("@")[0] ? msg?.key?.remoteJidAlt.split("@")[0] : msg?.key?.participant.split("@")[0];
         
         await sleep(1500);
         await sock.readMessages([msg?.key]);
      console.log(`${cyanB}Automatically Viewed update =>${reset}${magenta} ${user}${reset}`);
...
`;
  ...
  return await sock.sendMessage(jid,{ text:AmLost},{ quoted:msg})          
     ...
     /* console.log(configFetchJs()?.cypherAi?.enabled && isFromMe && text.trim().toLowerCase().split(" ")[0] !== configFetchJs().prefix+"cypherai off") */
       ...
       if(configFetchJs()?.cypherAi?.enabled === true && !isFromMe && text.trim()?.toLowerCase() !== configFetchJs()?.prefix+"cypherai off"){ 
 
           
           if(configFetchJs().cypherAi?.allowInGroups === false && msg?.key?.remoteJid?.endsWith("@g.us") ){}
      else if(configFetchJs().cypherAi.allowInPrivateChats === false && msg?.key?.remoteJid.endsWith("@lid") || configFetchJs()?.cypherAi?.allowInPrivateChats === false && msg?.key?.remoteJid?.endsWith("@whatsapp.net")){}
           else if(configFetchJs().cypherAi.allowInChannels === false && msg?.key?.remoteJid?.endsWith("@newsletter") || configFetchJs().cypherAi.allowInChannels === false && msg?.key?.remoteJid.endsWith("@broadcast")){} 
  ...
  //continue system automations 
       ...
       if(!text.startsWith(configFetchJs().prefix) && configFetchJs().production){
 ...
 if(msgTypeCheck === "imageMessage" || msgTypeCheck === "videoMessage" || msgTypeCheck ==="stickerMessage" || msgTypeCheck ==="audioMessage"|| msgTypeCheck === "documentMessage" || msgTypeCheck === "ptvMessage"){
    ...
    //console.log(downloadType,extention)    
     ...
     var sizeToKb = msg?.message?.[specificMessageType]?.fileLength?.toNumber();
     
     var sizeToMb = sizeToKb/ (1024);
     var finalMediaSize = Math.floor(sizeToMb);
     
     
     if(finalMediaSize > 600){return;};
...
if (msg.messageStubType === 118 || msg.labels?.includes('fail') || msg.labels?.includes('PreKeyError')) { 
...
*/
  ...
  var fetchAllowed = fs.readFileSync(path.join(__dirname,"../JSONS","private_allowals.json"),"utf8");
    ...
    */        
 if(privateCheck && !msg.key.fromMe && msg.key?.remoteJid !== AdminJid &&!(JSON.parse(fs.readFileSync(path.join(__dirname,"../JSONS","private_allowals.json"))).includes(msg?.key?.remoteJidAlt)))return;
 ...
 if(text === configFetchJs().prefix+"menu"){
    logCommand("Menu")
      ...
      logCommand("Prefix")
            if(!msg.key.fromMe){
  return await sock.reply(jid,"You do not have the owner's right to change my prefix",msg);
    ...
    return await sock.reply(jid,`${value} is already set as the prefix `,msg)
           ...
           let oldConfig =   configFetchJs();
            ...
            await sock.reply(jid,`My prefix has been changed to ${await oldConfig.prefix} successfully.`);
                
                
  }else if(text.toLowerCase().trim().startsWith( configFetchJs().prefix+"mode")){
      logCommand("Mode")
            if(!msg.key.fromMe/* || msg.key?.remoteJid !== AdminJid*/){
    return await sock.reply(jid,"You do not have the Admin right to change my mode ",msg)
}
 ...
 return await sock.reply(jid,"*Modes can only be public or private*",msg)
  ...
  return await sock.sendMessage(jid,{ text :  `~Already in ${modeValue} mode, ${msg.key.pushName || configFetchJs().owner || "user"}~`})
         ...
         //console.log(modeValue.trim().toLowerCase() === newConfigJson.private);
           ...
           console.log(`\x1b[1;7;33mMy private mode has been changed to ${await newConfigJson.private} by ${configFetchJs().owner}.\x1b[0m`)
   ...
   await repo(sock,jid,msg);
      ...
      "Accept":"application/json"
                ...
                if(fsFetchJson("..","package.json").version === newVersion){
                    if(!msg.key.fromMe){
                     return await sock.sendMessage(jid, { 
        text: `\`\`\`${configFetchJs().owner} is currently running cypher-md@${fsFetchJson("..","package.json").version} Latest\`\`\`` });                }else{
    return await sock.sendMessage(jid, { 
...
await sock.sendMessage(jid, { 
   ...
   var anti_del_value =  text.split(" ")[1];
          if(!anti_del_value){
     return await sock.reply(jid,`*⚠️Wrong command usage, try :${configFetchJs().prefix}antidelete on to toggle antidelete to on or ${configFetchJs().prefix}antidelete off to toggle antidelete to off*`,msg);
};
                if(anti_del_value === "on" && configFetchJs().antidelete === true){
   return await sock.reply(jid,"*Antidelete feature has already been enabled*");
  }else if(anti_del_value === "off" && configFetchJs().antidelete === false){
   return await sock.reply(jid,"*Antidelete feature has already been disabled*")
       ...
       anti_del();   
          ...
          await sock.reply(jid,`*Antidelete has been ${toggledTo} successfully👊🏾. Type "${configFetchJs().prefix}help antidelete" to get information about antidelete*`);
}
            ...
            else if(text.trim().toLowerCase().startsWith(configFetchJs().prefix+"obfuscate")){
...
*/
                if(!secondVar){
   return sock.reply(jid,`*The level of obfuscation was not provided eg. low,high,medium*`,msg);
}
               //console.log(level);
           ...
           logCommand("Obfuscate -high")         
             ...
             return sock.reply(jid,`*No JavaScript file received, for obfuscation, please reply ${configFetchJs().prefix}obfuscate ${secondVar} with a reply to a javascript having a ".js" extention*`,msg);
                    
                }   
        
                var isExtendedMessage = getContentType(msg.message);
                if(isExtendedMessage !== "extendedTextMessage"){
   return sock.reply(jid,`*please reply to a JavaScript file or a file ending with a ".js" extention*`,msg);
}
     
                    
                  
                    if (getContentType(msg?.message?.extendedTextMessage?.contextInfo?.quotedMessage) !== "documentMessage"){
       /* console.log(getContentType(msg?.message?.extendedTextMessage?.contextInfo?.quotedMessage) !== "documentMessage")*/
       
    return sock.reply(jid,`*please make sure to  reply to a JavaScript file or a file ending with a ".js" extention only*`,msg);
 
}            
                    
                    
          var isSuccess = await obf(sock,msg,jid,secondVar.replace("-","").trim().toLowerCase());
                    var tobuffer = Buffer.from(isSuccess.buffer);
                    if(isSuccess){
                        await sock.sendMessage(jid,{ 
           document: tobuffer,
           mimetype : "application/javascript",
           fileName: isSuccess.name                     },msg);
                        
                    }else{ 
                   return await sock.reply(jid,"*Obfuscation completly failed the code might have been an invalid JavaScript code.*")
                         }
   ...
   return sock.reply(jid,`*please reply to a JavaScript file or a file ending with a ".js" extention*`,msg);
    ...
    var isSuccess2 = await obf(sock,msg,jid,secondVar.replace("-","").trim().toLowerCase());
   ...
   /* console.log(getContentType(msg?.message?.extendedTextMessage?.contextInfo?.quotedMessage))*/
    ...
    var isSuccess3= await obf(sock,msg,jid,secondVar.replace("-","").trim().toLowerCase());
...
console.log(info)*/
    ...
    return await sock.reply(jid,`\`\`\`⚠wrong command usage: try ${configFetchJs().prefix}obfuscate -low <reply_to_js_file> or  ${configFetchJs().prefix}obfuscate -medium <reply_to_js_file> or ${configFetchJs().prefix}obfuscate -high <reply_to_js_file>with increasing security.   "${jsCodeStructuring[0]} ${jsCodeStructuring[1]}" is wrong.. ⚠\`\`\``,msg);
  }
        ...
        // await obf(obfuscationCode,level.trim().toLowerCase().replace("-",""));
            ...
            //update system manual
                ...
                if(configFetchJs().production === false){
   return await sock.reply(jid,`\`\`\`please toggle production to true to be able to use this bot in production mode: Toggling to false means You are the developer of this bot, You are not the developer of this bot\`\`\``,msg);
}
          ...
          else if(text.trim().toLowerCase() === configFetchJs().prefix+"vv"){
    ...
    var isExtended = msg.message?.extendedTextMessage?.contextInfo?.quotedMessage;
              
              if(!isExtended){
   return sock.reply(jid,"*please reply to a viewonce media to see the magic*",msg)
         }             
              
              
   var isReallyViewOnce= isExtended?.imageMessage?.viewOnce ||isExtended?.videoMessage?.viewOnce||isExtended?.audioMessage?.viewOnce;
              
            var viewType = isExtended?.imageMessage || isExtended?.videoMessage || isExtended?.audioMessage;
  ...
  if(trueViewOnce === "no"){
    return await sock.sendMessage(jid,{text: "*Expected type media of viewonce but received undefined, please reply to a viewonce message...*"})
   };
...
if(isExtended){
 ...
 var downloadBuffer = await downloadMediaMessage(
  ...
  var downloadedBuffer = await downloadBuffer;
      ...
      return await sock.sendMessage(jid,{ text: "*Failed to extract viewonce media...*"})
    }
}
       await sock.sendMessage(jid,{ react:{ text:'⚡', key:msg.key}});
              
              
              var get_expiration = msg.message?.extendedTextMessage?.contextInfo?.expiration;
  ...
  return await sock.sendMessage(myJid,{
     ...
     await sock.sendMessage(myJid,{
        [mainMedia] : downloadBuffer,
         ...
         else if(text.trim().toLowerCase().startsWith(configFetchJs().prefix+"add")){
             try{
    logCommand("Add User");
             var fullCommand = text.split(" ");
             var [ startCommand, ...args] = fullCommand 
            if(args.length === 0){
    return await sock.reply(jid,`*Wrong command usage: try ${configFetchJs().prefix}add <Phone_Numbers_without_the_+_sign>*`,msg)           
   ...
   for(const arg of args){
     if(arg.length < 10){
         return await sock.reply(jid,`*${arg} is supposed to be a valid whatsapp number, aborting...*`,msg);   
     }else if(!mobileTest.test(arg)){
   return await sock.reply(jid,`*${arg} is not a valid whatsapp number,aborting...*`,msg)
        ...
        //autotyping command    
            else if(text.trim().toLowerCase().startsWith(configFetchJs().prefix+"autotyping")){
     var fullcommand = text.split(" ");
                //console.log(text);
                
               // var toggler = fullCommand[1];
                if(!text.split(" ")[1]){
  return await sock.reply(jid,`*Expected a toggler.Type ${configFetchJs().prefix}help -autotyping for autotyping details and how to use*`,msg);
}
 ...
 await sock.reply(jid,`*Unexpected token. Type ${configFetchJs().prefix}help -autotyping for autotyping details and how to use*`,msg);
       ...
       else if(text.trim().toLowerCase().startsWith(configFetchJs().prefix+"autorecording")){
           try{
    var fullCommandAR = text.split(" ");
    var [ record , argsRecord ] = fullCommandAR;  
           logCommand("Automatically set recording presence on message");
           
 if(record.toLowerCase()!== `${configFetchJs().prefix}autorecording`){
   return await sock.reply(jid,log(`~Invalid command structure, did you mean ${configFetchJs().prefix}autorecording <mode (on/off)~`));
 }
...
*/
           if(!argsRecord){
     return await sock.reply(jid,log(`~Toggle format returned undefined, please add the mode <on/off> when using ${record}~`),msg);
  ...
  return await sock.reply(jid,log(`~Invalid toggle format, type ${configFetchJs().prefix}help -autorecording for autorecording command usage and try again~`),msg);
}
       }catch{
   await sock.reply(jid,log(`❌sorry but autorecording system failed to initiate`),msg)
 }
}
            ...
            else if(text.trim().toLowerCase() === configFetchJs().prefix+"setkeys"){
      if(!msg.key.fromMe){ return await sock.sendMessage(jid,{ text:`\`\`\`You do not have Admin right to access my encryption keys\`\`\``})}
     return await setKeys(sock,myJid,jid,msg);
}
            
            
            
        else if(text.trim().toLowerCase().startsWith(configFetchJs().prefix+"encrypt")){
            try{
var fullCommand = text?.split(" ");
    
var [ first, toSwitch, ...args ] = fullCommand;
    if(!toSwitch){
        return await sock.reply(jid,`\`\`\`Wrong command usage, try: ${configFetchJs().prefix}encrypt -hex <encryption_body> or ${configFetchJs().prefix}encrypt -base64 or type ${configFetchJs().prefix}help -encrypt for usage\`\`\``)            
    }
                
                
                if(args.length < 1){
   return await sock.reply(jid,`\`\`\`Expected a value but recieved undefined, please reply ${configFetchJs().prefix}help -encrypt for command usage\`\`\``)
  ...
  */   
     ...
     var iv =  credentials.iv;    /*console.log(credentials);*/
          ...
          return;*/
                ...
                if(!key || !iv){
   return await sock.reply(jid,`*Encryption credentials where not found, please reply ${configFetchJs().prefix}setkeys  to generate new credentials for encryption*`,msg);
 };
 ...
 switch(toSwitch){
     case "-hex":
         ...
         var encryption =cipher.update(args.join(" "),"utf8","hex");
       encryption += cipher.final("hex");
                console.log(`[\x1b[34mEncrypting Data...\x1b[0m]`)
            if(encryption){
                console.log(`[\x1b[34m Encryption was successfull...\x1b[0m]`);
                
               return await sock.sendMessage(jid,{ text:encryption},{ quoted:msg});
                
                  
                
            }else{   return await sock.reply(jid,"*Could not encrypt value to hex, are you sure it is a readable value*",msg);
                 };                  
         break;
     ...
     case "-base64":
         ...
         var encryption2 =cipher.update(args.join(" "),"utf8","base64");
       encryption2 += cipher.final("base64");
                console.log(`[\x1b[36mEncrypting Data...\x1b[0m]`)
            if(encryption2){
                console.log(`[\x1b[36m Encryption was successfull...\x1b[0m]`);
                
               return await sock.sendMessage(jid,{ text:encryption2},{ quoted:msg});
                
                  
            }else{   return await sock.reply(jid,"*Could not encrypt value to base64, are you sure it is a readable value*",msg);
             }
   ...
   return await sock.reply(jid,"*Encryption failed due to an internal error, please try again or make sure to update the system*",msg);
 }         
}
            ...
            else if(text.trim().toLowerCase() === configFetchJs().prefix+"getkeys"){
                
  if(!msg?.key?.fromMe){ 
   return await sock.reply(jid,"*You do not have admin rights to access my sensitive keys*");
};
    return await getKeys(sock,myJid,jid,jidAlt,msg)      
}
                     
            
            else if(text.trim().toLowerCase().startsWith(configFetchJs().prefix+"decrypt")){
                try{
...
var [ first, toSwitch, ...args ] = fullCommand;
    if(!toSwitch){
        return await sock.reply(jid,`\`\`\`Wrong command usage, try: ${configFetchJs().prefix}decrypt -hex <encryption_body> or ${configFetchJs().prefix}decrypt -base64 or type ${configFetchJs().prefix}help -decrypt for usage\`\`\``)            
    }
                
                
                if(args.length < 1){
   return await sock.reply(jid,`\`\`\`Expected a value but recieved undefined, please reply ${configFetchJs().prefix}help -decrypt for command usage\`\`\``)
     ...
     var iv =  credentials.iv;    /*console.log(credentials);*/
          ...
          return;*/
                ...
                if(!key || !iv){
   return await sock.reply(jid,`\`\`\` Decryption credentials where not found, please reply ${configFetchJs().prefix}setkeys  to generate new credentials for encryption*\`\`\``,msg);
 };
  switch(toSwitch){
      case "-hex":
          logCommand("Decryption from hex to utf8");
          var decrypted = decipher.update(args.join(" "),"hex","utf8");
          decrypted += decipher.final("utf8");
          
          if(decrypted){
               return await sock.sendMessage(jid,{ text:decrypted},{ quoted:msg})
          
}else{
  return await sock.reply(jid," ```*Could not decrypt to human readable, are you sure it is a hexadecimal*```",msg);
}
          
          break;
      case "-base64":
          logCommand("Decryption from base64 to utf8");
          var decrypted2 = decipher.update(args.join(" "),"base64","utf8");
          decrypted2 += decipher.final("utf8");
          
          if(decrypted2){
               return await sock.sendMessage(jid,{ text:decrypted2},{ quoted:msg})
          
}else{
  return await sock.reply(jid,`*\`\`\`Could not decrypt to human readable, are you sure it is a base64\`\`\`*`,msg);
};        
          
                    break;
      default:
          return await sock.reply(jid,`*\`\`\`Wrong command usage, try: ${configFetchJs().prefix}decrypt -hex <encrypted_body> or ${configFetchJs().prefix}decrypt -base64 or type ${configFetchJs().prefix}help -decrypt for usage*\`\`\`*`); 

}   
                    
                }catch(e){
                    return await sock.reply(jid,`*\`\`\`Failed to decrypt, please make sure the same credentials used to encrypt these values are the same credentials for decrypting and also make sure to choose between hex and base64 reapectfully\`\`\`*`,msg);
          ...
          else if(text.trim().toLowerCase().startsWith(configFetchJs().prefix+"help")){
      var fullCommand = text.split(" ");
var [ first, main ] = fullCommand;
             if(first !== configFetchJs().prefix+"help"){
   return await sock.reply(jid,`*\`\`\`Invalid command, did you mean ${configFetchJs().prefix}help -<command_you_need_help_with>\`\`\`*`);
}
          if(!main){
                        
   return await sock.sendMessage(jid,{ text:`*\`\`\`please include the type of command you need help with example ${configFetchJs().prefix}help -ping, make sure to include a space and the "-" symbol in every help call\`\`\`*`},{ quoted:msg});
          };
              if(!main.startsWith("-")){
   return await sock.reply(jid,`*\`\`\`Invalid command help structure, expected a "-" but received undefined. Did you mean, ${configFetchJs().prefix}help -${main}\`\`\`*`,msg);
}
              
         return await sock.sendMessage(jid,{text:help(main)},{quoted:msg});
            ...
            else if(text.trim().toLowerCase().startsWith(`${configFetchJs().prefix}sha256`)){
                logCommand("SHA256")
          var fullCommandSha256 = text.split(" ");
                ...
                if(first.toLowerCase() != configFetchJs().prefix+"sha256"){
   return sock.reply(jid,log(`invalid command usage, did you mean ${configFetchJs().prefix}sha256`),msg);

}
                if(!args || args.length < 1){
   return sock.reply(jid,log(`Expected an argument body but received undefined near ${configFetchJs().prefix}sha256 [], please provide a value to work on...`,msg));
...
*/
      ...
      var SHA256HASH = crypto.createHash("sha256").update(fullValue).digest("hex");
              console.log(`${lime}Hash created successfully${reset}${blueB} =>> SHA256${reset}`);
                    console.log(fullValue)
        return await sock.reply(jid,`*\`\`\`${SHA256HASH}\`\`\`*`,msg)
   ...
   return Err(sock,jid,msg,first)
          ...
          else if(text.trim().toLowerCase().startsWith(`${configFetchJs().prefix}sha512`)){
          ...
          var [ first, ...args ] =  fullCommandSha512;
              ...
              try{
              logCommand("SHA512");
                if(first.toLowerCase() != configFetchJs().prefix+"sha512"){
   return sock.reply(jid, log(`invalid command usage, did you mean ${configFetchJs().prefix}sha512`),msg);

}
                if(!args || args.length < 1){
   return sock.reply(jid,log(`Expected an argument body but received undefined near ${configFetchJs().prefix}sha512 [], please provide a value to work on...`,msg));
 
};
                  
          var fullValue2 = args.join(" ");
      var SHA512HASH = crypto.createHash("sha512").update(fullValue2).digest("hex");
              console.log(`${lime}Hash created successfully${reset}${blueB} =>> SHA512${reset}`);
                    /*
console.log(fullValue)
*/
        return await sock.reply(jid,`*\`\`\`${SHA512HASH}\`\`\`*`,msg)        
       ...
       else if(text?.trim()?.toLowerCase()?.startsWith(configFetchJs().prefix + "autoviewstatus")){
    if(!msg?.key?.fromMe){
     return await sock.reply(jid,log(`~You do not have admin rights to interfere with this command~`),msg);
}
  var fullCommandATV = text.split(" ");
          
     var [ first, second ] = fullCommandATV;
           
    if(first.toLowerCase() !== configFetchJs().prefix+"autoviewstatus"){
      return sock.reply(jid,log(`Invalid command, did you mean ${configFetchJs().prefix}autoviewstatus <on_or_off>`),msg);             
    };
           
    if(!second){
   return await sock.reply(jid,log(`The command is invalid, try : ${configFetchJs().prefix}autoviewstatus <and_the_mode(on or off)>`));
 }
           /*
console.log(second.trim().toLowerCase() !== "on")
*/
           if(second.trim().toLowerCase() === "on" || second.trim().toLowerCase() === "off"){
               
               toggleStatusView(sock,jid,msg,second);
           }else{
return await sock.reply(jid,log(`The toggler for autoviewstatus can only be [on / off]`),msg);

 }
}
       
       
       else if(text?.trim()?.toLowerCase()?.split(" ")[0] === configFetchJs().prefix+"gpt5"){
   ...
   var [ first, ...args ] = fullCommandCypherAi;
     ...
     if(!args || args.length < 1){
   return await sock.reply(jid,log(`Could not find a question, please reply with a question`),msg);
};
           
    var gpt5Fetch = await fetch(`https://apis.davidcyriltech.my.id/ai/gpt-5?prompt=${encodeURIComponent(args.join(" "))}`);
    var gpt5Response = await gpt5Fetch.json();
       ...
       else if(text?.trim()?.toLowerCase()?.split(" ")[0] === configFetchJs().prefix+"gpt5.5"){
           logCommand("ChatGPT version 5.5")
  return await returnAi(text,"https://apis.davidcyriltech.my.id/ai/gpt-5.5","gpt5.5","prompt","data",sock,jid,msg)

}
       
       
       
       else if(text?.trim()?.toLowerCase()?.split(" ")[0] === configFetchJs().prefix+"gpt5-nano"){
           logCommand("ChatGPT version 5-nano")
  return await returnAi(text,"https://apis.davidcyriltech.my.id/gpt-5-nano","gpt5-nano","prompt","data",sock,jid,msg)
     ...
     else if(text?.trim()?.toLowerCase()?.split(" ")[0] ===configFetchJs().prefix+"cypherai"){
         logCommand("CypherAi modified")
 if(!msg?.key?.fromMe){
   return await sock.reply(jid,log(`~${msg?.pushName}, You are not allowed to commit any changes to my setup, please contact the Admin~`),msg);
}
   var fullCommandCypherAi = text.split(" ");
  var [ first, args ] = fullCommandCypherAi;
    if(!args || args.length < 1){
       return await sock.reply(jid,log(`Invalid command structures please include a toggler. eg. ${configFetchJs().prefix}cypherAi <on/off>`),msg);
}
    var finalToggler = args.trim().toLowerCase() === "on" ? true : args.trim().toLowerCase() === "off" ? false : null; 
       //console.log(finalToggler);
        ...
        else if(text?.trim()?.toLowerCase()?.split(" ")[0] === configFetchJs().prefix+"grok4.1"){
           logCommand("GROK version 4.1");
            
  return await returnAi(text,"https://apis.davidcyriltech.my.id/ai/grok-4.1-fast","grok4.1","prompt","data",sock,jid,msg)

}
    
       
       
       else if(text?.trim()?.toLowerCase()?.split(" ")[0] === configFetchJs().prefix+"claudeopus-4.5"){
           logCommand("Claude Opus-4.5");
            
  return await returnAi(text,"https://apis.davidcyriltech.my.id/ai/claude-opus-4.5","claudeopus-4.6","prompt","data",sock,jid,msg)

}
       
       
       
       else if(text?.trim()?.toLowerCase()?.split(" ")[0] === configFetchJs().prefix+"deepseek"){
           
 logCommand("Deepseek version-4");
            
  return await returnAi(text,"https://apis.davidcyriltech.my.id/ai/deepseek-v4-flash","deepseek","prompt","data",sock,jid,msg)

}
       
       
       
       else if(text?.trim()?.toLowerCase()?.split(" ")[0] === configFetchJs().prefix+"qwen3"){
           
 logCommand("qwen3");
            
  return await returnAi(text,"https://apis.davidcyriltech.my.id/ai/qwen3-max","qwen3","prompt","data",sock,jid,msg)

}
       
       
      else if(text.trim().toLowerCase().split(" ")[0] === configFetchJs().prefix+"news"){
